...
document.addEventListener("DOMContentLoaded", function() {
    var condicionamentoform = $("#formulario-condicionamento");
    if (condicionamentoform.length) {
        condicionamentoform.submit(function (event){
            var valorItems = itemsCadastro.val();
            if (valorItems == "") {
                event.preventDefault();
                return false;
            }else {
                localStorage.setItem("condicionamento", valorItems);
            }
        })
        var itemsCadastro = condicionamentoform.find("[name='items']");
        var valorItems = localStorage.getItem("condicionamento");
        if (valorItems) {
            itemsCadastro.val(valorItems);
            var toactivate = document.querySelector(".items-grid4 li[data-id='"+valorItems+"']");
            if (toactivate) {
